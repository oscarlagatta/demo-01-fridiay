...
const CustomNode = ({ data, id }: NodeProps<CustomNodeType>) => {
  ...
  const txActive = multiContextSearch?.hasResults || false
  const txFetching = multiContextSearch?.isFetching || false
  const matchedAitIds = multiContextSearch?.matchedAitIds || new Set<string>()

  const showTable = async (aitNum: string) => {
    console.log("[v0] Table view requested for AIT:", aitNum)
    // TODO: Implement table view functionality with the new context system
  }
